...
function build() {
  const merchantId = process.env.SOUTHBILL_MERCHANT_ID ?? '';
  requireCondition(process.env.SOUTHBILL_MODE === 'live', 'LIVE_ACCOUNT_CONFIGURATION_REQUIRED');
  const account = { merchantId, livemode: true };
